...
abstract class MultiKeyPipelineBase extends PipelineBase implements
    ...
    public Response<byte[]> ds_hset(byte[] key, byte[] field, byte[] value) {
    	this.client.ds_hset(key, field, value);
        return getResponse(BuilderFactory.BYTE_ARRAY);
    }
    
	public Response<byte[]> ds_hget(byte[] key, byte[] field) {
		this.client.ds_hget(key, field);
		return getResponse(BuilderFactory.BYTE_ARRAY);
	}
	public Response<byte[]> ds_get(byte[] key) {
		this.client.ds_get(key);
		return getResponse(BuilderFactory.BYTE_ARRAY);
	}
	public Response<byte[]> ds_set(byte[] key, byte[] val) {
		this.client.ds_set(key, val);
		return getResponse(BuilderFactory.BYTE_ARRAY);
	}
}
